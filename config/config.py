...
class Config:
    ...
    # Directories - handle PyInstaller frozen state
    ...
    # Use DATA_ROOT for writable directories (jobs, storage, uploads)
    # Convert to absolute paths if they're relative
    JOBS_DIR = str(DATA_ROOT / _jobs_dir) if not Path(_jobs_dir).is_absolute() else _jobs_dir
    STORAGE_DIR = str(DATA_ROOT / _storage_dir) if not Path(_storage_dir).is_absolute() else _storage_dir
    UPLOAD_DIR = str(DATA_ROOT / _upload_dir) if not Path(_upload_dir).is_absolute() else _upload_dir
